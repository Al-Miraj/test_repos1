...
class ReservationSystem
{
    ...
    public void DisplayTablesMap(int currentX, int currentY)
    {
        Console.WriteLine("Visual Map of Tables:");
        Console.WriteLine("Green: Available | Red: Reserved | Yellow: Selected");
        for (int y = 1; y <= 5; y++) // Assuming 5 is the max Y-coordinate
        {
            for (int x = 1; x <= 3; x++) // Assuming 3 is the max X-coordinate
            {
                // Find the table at the x, y coordinate
                Table table = Tables.FirstOrDefault(t => t.Coordinate.Item1 == x && t.Coordinate.Item2 == y);
                if (table != null)
                {
                    // Set the background color based on the table's state
                    ...
                    // Write the table number with padding for alignment
                    Console.Write($" {table.TableNumber:D2} ");
                    Console.ResetColor(); // Reset the color for the next table
                }
            }
            Console.WriteLine(); // New line for the next row of tables
        }
    }
...
class ReservationSystem
{
    ...
    public Table GetChosenTable()
    {
        ...
        {
            Console.Clear();
            DisplayTablesMap(currentX, currentY);
            selectedTable = ShowSelectedTable(currentX, currentY);
            TableSelectionFeedback(selectedTable);

            keyInfo = Console.ReadKey();
            if (keyInfo.Key == ConsoleKey.UpArrow && currentY > 1)
            {
                currentY--;
            }
            else if (keyInfo.Key == ConsoleKey.DownArrow && currentY < 5) // Assuming 5 is the max Y-coordinate
            {
                currentY++;
            }
            else if (keyInfo.Key == ConsoleKey.LeftArrow && currentX > 1)
            {
                currentX--;
            }
            else if (keyInfo.Key == ConsoleKey.RightArrow && currentX < 3) // Assuming 3 is the max X-coordinate
            {
                currentX++;
            }
            else if (keyInfo.Key == ConsoleKey.Enter)
            {
                if (!selectedTable.IsReservated && Reservation.NumberOfPeople <= selectedTable.Capacity)
                {
                    selectedTable.IsReservated = true;
                    WriteToFile(Tables, TablesJson);
                    break;
                }
            }
